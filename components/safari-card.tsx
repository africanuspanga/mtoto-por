...
export function SafariCard({ safari, index }: SafariCardProps) {
      ...
      <Card className="group overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-500 bg-card">
        ...
        <div className="relative h-56 sm:h-64 overflow-hidden">
          ...
          <div className="absolute top-4 right-4">
            <span className="bg-primary text-primary-foreground px-3 py-1.5 rounded-full text-sm font-bold shadow-lg">
              From {startingPrice}
            </span>
          </div>
        ...
        <CardContent className="p-5 sm:p-6">
          ...
          <p className="text-primary font-medium text-sm mb-3">{safari.subtitle}</p>

          {/* Summary */}
          <p className="text-muted-foreground text-sm leading-relaxed mb-4 line-clamp-2">{safari.summary}</p>

          {/* Wildlife Tags */}
          <div className="flex flex-wrap gap-1.5 mb-5">
            {safari.wildlife.slice(0, 4).map((animal) => (
              <span key={animal} className="text-xs bg-secondary text-secondary-foreground px-2 py-1 rounded-full">
                {animal}
              </span>
            ))}
            {safari.wildlife.length > 4 && (
              <span className="text-xs bg-secondary text-secondary-foreground px-2 py-1 rounded-full">
                +{safari.wildlife.length - 4} more
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button asChild variant="default" className="flex-1 group/btn">
              <Link href={`/tanzania-safaris/${safari.slug}`} onClick={() => window.scrollTo(0, 0)}>
                Read More
